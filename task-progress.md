...
- 公网验证：GitHub Pages 已完成当前 `main` 分支部署。2026-08-17 21:50 通过带缓存绕过的公网数据请求和浏览器 DOM 回查，六张卡片均渲染为 `a.insight-card`，链接数量为 6、空链接数量为 0。
- 用户侧刷新：已经打开的旧网页标签不会自动重新执行内容请求；重新打开网站或执行一次强制刷新后，即可获得已部署的可点击卡片。
